...
function reportError(error, state) {
  ...
  function parseInstancesList(instancesList, state) {
    tabularize(instancesList.Reservations, state);
    return(instancesList);
  }

  function tabularize(instancesList, state) {
    state.table = new Table({ head: ['','box', 'dev', 'state', 'started'],
                              style: { compact: true, border: [], head: [] } });
    var rows = rowsForTable(instancesList, state);
    rows.forEach(function(row){ state.table.push(row) });
  }

  function rowsForTable(instancesList) {
    return _.map(instancesList, generateTableRow);
  }

  function generateTableRow(reservation, index) {
    var instance, tags, box, dev, instanceState, started;
    instance = reservation.Instances[0];
    tags = instance.Tags;
    box = tagValue(instance.Tags, 'Name');
    dev = tagValue(instance.Tags, 'Requestor');
    instanceState = instance.State.Name;
    started = instance.LaunchTime.toString();
    return [index + 1, box, dev, instanceState, started];
  }
