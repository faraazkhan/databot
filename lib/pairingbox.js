...
var EC2 = new sdk.EC2({region: region});
...
function reportError(error, state) {
  ...
  function runStopInstance(ec2Response) {
    var params = instanceParams(ec2Response.Reservations);
  ...
  function isStale(reservation) {
    return (instanceAge(reservation) > 21 || idleSince(reservation) > 7); // more than 2 days since launch
  }
  ...
  function launchTimeForReservation(reservation) {
    return moment(reservation.Instances[0].BlockDeviceMappings[0].Ebs.AttachTime);
  }
